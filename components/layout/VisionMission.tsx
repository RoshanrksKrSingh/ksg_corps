...
const VisionMission = () => {
  ...
  return (
    // ✅ SWAPPED BG: Starts Teal (#0F333D) -> Ends Dark Blue (#09102d)
    // Matches the bottom of AboutUs perfectly.
    <section className="relative py-10 overflow-hidden bg-slate-50 dark:bg-gradient-to-b dark:from-[#0F333D] dark:to-[#09102d] transition-colors duration-300">
        ...
        >
          {cards.map((item, i) => (
            ...
            >
              ...
              <div className={`absolute inset-[-50%] ${item.rotateGradient} animate-spin-border`}></div>
              ...
              <div className="relative z-20 flex flex-col p-8 h-full w-full">
                <div className={`absolute -top-4 -right-4 opacity-5 dark:opacity-[0.03] group-hover:opacity-10 transition-opacity rotate-12 ${item.iconColor}`}>
                  <item.icon size={160} />
                </div>

                <div className={`w-16 h-16 mb-6 rounded-2xl flex items-center justify-center border backdrop-blur-sm transition-all group-hover:scale-110 ${item.iconBg} ${item.iconColor} shadow-lg ${item.iconShadow}`}>
                  <item.icon size={32} />
                </div>
