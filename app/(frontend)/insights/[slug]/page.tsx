          ...
          <div className="flex flex-wrap items-center gap-6 text-sm md:text-base font-medium text-gray-600 dark:text-gray-300 mb-10">
            <div className="flex items-center gap-2">
              ...
              <span className="text-green-700 dark:text-green-300 font-bold">
                {blog.author || "KSG Team"}
              </span>
            </div>
            <div className="flex items-center gap-2">
              <Calendar size={18} className="text-gray-400" />
              <span>
                Last updated on {formatDate(blog.updatedAt || blog.createdAt)}
              </span>
            </div>
            <div className="flex items-center gap-2">
            ...
            {blog.content.map((block: any, index: number) => {
              ...
              if (block.type === "heading") {
                return (
                  <h2
                    key={index}
                    className="text-2xl md:text-2xl 2xl:text-3xl font-extrabold mt-12 mb-4 text-transparent bg-clip-text bg-gradient-to-r from-[#041D2D] to-green-600 dark:from-white dark:to-green-400"
                  >
                    {block.value}
                  </h2>
                );
              ...
              if (block.type === "table") {
                ...
                return (
                  <div
                    key={index}
                    className="my-10 overflow-x-auto rounded-2xl border border-gray-200 dark:border-white/10 shadow-lg"
                  >
                          ...
                          >
                            {row
                              ...
                              .map((cell: string, cellIndex: number) => (
                                <td
                                  key={cellIndex}
                                  className="px-6 py-4 whitespace-nowrap text-gray-700 dark:text-gray-300 font-medium"
                                >
                                  {cell.trim()}
                                </td>
                              ))}
              ...
              return (
                ...
                <p
                  key={index}
                  className="text-base md:text-sm 2xl:text-base text-slate-700 dark:text-gray-300 leading-7 md:leading-8 mb-6 font-normal"
                >
                  {block.value}
                </p>
              );
