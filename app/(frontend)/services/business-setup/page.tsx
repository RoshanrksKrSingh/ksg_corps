         ...
         <section className="relative z-10 w-full py-20">
            <div className="w-[99%] max-w-8xl 2xl:max-w-[95%] mx-auto px-4 lg:px-8">
                ...
                <motion.div 
                  ...
                  viewport={{ once: true }}
                >
                    <h2 className="text-2xl md:text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-green-400 to-blue-500 transition-colors">Our Business Setup/Corporate PRO includes</h2>
                ...
                >
                    {services.map((s) => (
                        <motion.div key={s.id} variants={cardVariant}>
                          <Link href={`/services/business-setup/${s.id}`} className="group relative block p-8 bg-white dark:bg-white/5 backdrop-blur-md border border-gray-200 dark:border-white/10 rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 overflow-hidden h-full">
